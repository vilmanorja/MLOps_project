...
"""
    ...
    for c in ['NoOfDependents', 'SegmentId', 'IndustryId', 'LegalDocName1Id', 'CustomerId', 'Duration_Months', 'NumberOfInstallmentsToPay', 'HasDefault',
       'run_date', 'Previous_Loan_Count',  'Previous_Loan_Defaults', 'Active_Loans_Count', 'Age', 'Is_Employed', 'Is_Married']:
    ...
    # Cleaning (remove when cleaning pipeline is ready)
    def safe_parse1(val):
        try:
            if pd.isna(val):
                return None
    ...
    logger.info(f"Number of columns processed: {len(df_numeric.columns) + len(df_categorical.columns) + len(df_datetime.columns) + len(df_target.columns)} columns.")
    logger.info(f"{categorical_features}")
    ...
    df_validation = pd.concat([df_val1, df_val2, df_val3, df_val4], ignore_index=True)
